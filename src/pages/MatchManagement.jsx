...
export default function MatchManagement() {
  ...
  const [scoreData, setScoreData] = useState({ homeScore: 0, awayScore: 0, minute: 0 })

  useEffect(() => { checkCommissionerStatus() }, [user])
  ...
  const openScoreModal = (match) => {
    ...
    setScoreData({ homeScore: match.homeScore || 0, awayScore: match.awayScore || 0, minute: match.minute || 0 })
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold">Match Management</h1>
            <p className="text-gray-400 mt-1 text-sm">Schedule, update scores, and manage matches</p>
          </div>
          <button
            onClick={handleCreateMatch}
            className="bg-green-600 hover:bg-green-500 px-4 sm:px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2 transition text-sm sm:text-base"
          >
            <FaPlus /> <span className="hidden sm:inline">New Match</span><span className="sm:hidden">New</span>
          </button>
        </div>
        ...
        {leagues.length > 1 && (
          <div className="mb-6">
            ...
            <select
              ...
              className="bg-gray-900 border border-gray-700 rounded-xl px-4 py-2 text-white text-sm w-full sm:w-auto"
            >
              {leagues.map(league => (
                <option key={league._id} value={league._id}>{league.name} ({league.sport})</option>
              ))}
            </select>
          </div>
        )}
                ...
                <div className="p-4 sm:p-5">
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                      ...
                      {match.week && <span className="text-gray-500 text-xs">GW{match.week}</span>}
                      {match.minute > 0 && <span className="text-yellow-500 text-sm font-mono">{match.minute}'</span>}
                    </div>
                  ...
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <div className="flex-1 text-center">
                      <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-2">
                        <span className="text-base sm:text-xl font-bold">{match.homeTeam?.name?.[0] || '?'}</span>
                      </div>
                      <p className="font-semibold text-sm">{match.homeTeam?.name || 'TBD'}</p>
                      <p className="text-2xl sm:text-3xl font-bold mt-2">{match.homeScore ?? 0}</p>
                    </div>
                    <div className="text-xl sm:text-2xl font-black text-gray-600 px-2 sm:px-4">VS</div>
                    <div className="flex-1 text-center">
                      <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-2">
                        <span className="text-base sm:text-xl font-bold">{match.awayTeam?.name?.[0] || '?'}</span>
                      </div>
                      <p className="font-semibold text-sm">{match.awayTeam?.name || 'TBD'}</p>
                      <p className="text-2xl sm:text-3xl font-bold mt-2">{match.awayScore ?? 0}</p>
                    </div>
                  </div>

                  {match.venue && (
                    <p className="text-xs sm:text-sm text-gray-500 text-center mb-4 flex items-center justify-center gap-1">
                  ...
                  <div className="flex gap-2 justify-end border-t border-gray-800 pt-4 flex-wrap">
                    ...
                    {match.status === 'live' && (
                      <>
                        <button
                          ...
                          className="px-3 sm:px-4 py-2 bg-yellow-600 hover:bg-yellow-500 rounded-lg text-xs sm:text-sm font-semibold"
                        >
                          HT
                        </button>
                        <button
                          onClick={() => handleMatchStatus(match._id, 'ft', match.minute)}
                          className="px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-xs sm:text-sm font-semibold"
                        >
                          FT
                        </button>
                      </>
                    ...
                    <button
                      ...
                      className="px-3 sm:px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-xs sm:text-sm font-semibold"
                    >
                      <FaEdit />
                    </button>
                    <button
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-gray-900 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">{editingMatch ? 'Edit Match' : 'Create New Match'}</h2>
            ...
            <form onSubmit={handleSubmitMatch} className="space-y-4">
              <div>
                ...
                <select required value={formData.homeTeam} onChange={(e) => setFormData({...formData, homeTeam: e.target.value})}
              ...
              <div>
                ...
                <select required value={formData.awayTeam} onChange={(e) => setFormData({...formData, awayTeam: e.target.value})}
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" required min="1" value={formData.week}
            ...
            <form onSubmit={handleUpdateScore} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" required min="0" value={scoreData.homeScore}
                    onChange={(e) => setScoreData({...scoreData, homeScore: parseInt(e.target.value)})}
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white text-center text-2xl" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Away Score</label>
                  <input type="number" required min="0" value={scoreData.awayScore}
