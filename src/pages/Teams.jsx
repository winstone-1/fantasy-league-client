  ...
  return (
    ...
    >
        ...
        {player ? (
          <>
            {player.photo ? (
              ...
              <span className="text-lg sm:text-xl">{sport === 'basketball' ? <FaBasketballBall /> : <FaFutbol />}</span>
            )}
      ...
      <div className="text-center">
        <div className="bg-black/60 backdrop-blur-md rounded px-1.5 py-0.5 border border-white/10">
          <span className="text-[9px] sm:text-[10px] font-bold text-gray-300 uppercase tracking-wider">{position.label}</span>
        </div>
        {player && (
          <>
            <p className="text-[10px] sm:text-[11px] font-medium text-white mt-1 max-w-[60px] sm:max-w-[70px] truncate drop-shadow-md">
              {player?.name?.split(' ').pop() || 'Player'}
            </p>
            <p className="text-[9px] text-gray-400 mt-0.5">{player?.position || ''}</p>
          </>
        )}
      </div>
    </div>
...
function SoccerPitch({ positions, players, onSlotClick, onDragStart, onDragOver, onDrop, onRemovePlayer }) {
  ...
  return (
    <div className="relative w-full rounded-3xl overflow-hidden border-4 border-white/10 shadow-2xl" style={{ minHeight: '420px', background: 'linear-gradient(180deg, #14532d 0%, #166534 50%, #14532d 100%)' }}>
      <div className="absolute inset-0 pointer-events-none opacity-20">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-36 sm:w-48 h-20 sm:h-24 border-2 border-white rounded-b-3xl" />
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-36 sm:w-48 h-20 sm:h-24 border-2 border-white rounded-t-3xl" />
        <div className="absolute top-1/2 left-0 right-0 border-t-2 border-white" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-28 sm:w-36 h-28 sm:h-36 border-2 border-white rounded-full" />
      </div>
      <div className="relative z-10 flex flex-col justify-around py-6 sm:py-8 h-full" style={{ minHeight: '420px' }}>
        {rows.map(row => (
          <div key={row} className="flex justify-around items-center px-2 sm:px-4">
            {positions.filter(p => p.row === row).map(pos => (
              <PlayerSlot key={pos.id} position={pos} player={players[pos.id]} sport="soccer"
                onClick={onSlotClick} onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop} onContextMenu={onRemovePlayer} />
            ))}
          </div>
        ))}
      </div>
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="relative w-full rounded-3xl overflow-hidden p-6 sm:p-10 border-4 border-white/10 shadow-2xl" style={{ minHeight: '220px', background: 'radial-gradient(circle, #9a3412 0%, #7c2d12 100%)' }}>
        ...
        <div className="relative z-10 flex justify-around items-center h-full flex-wrap gap-3 sm:gap-4">
          {starters.map(pos => (
            <PlayerSlot key={pos.id} position={pos} player={players[pos.id]} sport="basketball"
              onClick={onSlotClick} onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop} onContextMenu={onRemovePlayer} />
          ))}
        </div>
      </div>
...
export default function Teams() {
  ...
  const fetchTeams = async () => {
    try {
      ...
      let soccerTeamFound = null, nbaTeamFound = null, soccerPlayersMap = {}, nbaPlayersMap = {}
        ...
        try {
          ...
          if (myTeam) {
            ...
            if (myTeam.players && Array.isArray(myTeam.players)) {
              myTeam.players.forEach(item => { if (item.position && item.player) { mapped[item.position] = item.player._id ? item.player : item.player; } });
            }
            if (league.sport === 'soccer') { soccerTeamFound = { ...myTeam, leagueId: league._id }; soccerPlayersMap = mapped; }
            else if (league.sport === 'basketball') { nbaTeamFound = { ...myTeam, leagueId: league._id }; nbaPlayersMap = mapped; }
          }
        } catch (err) { console.error(`Error fetching teams for league ${league._id}:`, err) }
      }
      setSoccerTeam(soccerTeamFound); setNbaTeam(nbaTeamFound)
      setSoccerPlayers(soccerPlayersMap); setNbaPlayers(nbaPlayersMap)
  ...
  const handleCreateTeam = async () => {
    try {
      ...
      const response = await api.post(`/leagues/${targetLeague._id}/teams`, { name: `${user.username || 'My'} ${sport === 'soccer' ? 'FC' : 'Squad'}` })
      const positions = sport === 'soccer' ? FORMATIONS[soccerFormation] : NBA_LINEUPS[nbaLineup]
      const emptyRoster = positions.reduce((acc, pos) => { acc[pos.id] = null; return acc }, {})
      if (sport === 'soccer') { setSoccerTeam({ ...response.data, leagueId: targetLeague._id }); setSoccerPlayers(emptyRoster); }
      else { setNbaTeam({ ...response.data, leagueId: targetLeague._id }); setNbaPlayers(emptyRoster); }
      await fetchTeams()
    } catch (err) { console.error('Create team error:', err); alert(err.response?.data?.message || "Failed to create team.") }
  ...
  const openPicker = async (pos) => {
    ...
    } catch (err) { console.error('Error fetching players:', err); alert('Failed to load available players') }
  }
  ...
  const saveTeam = async () => {
    ...
    const roster = Object.entries(currentPlayers).filter(([_, player]) => player && player._id).map(([slotId, player]) => ({ position: slotId, player: player._id }));
    if (roster.length === 0) { alert("Please add at least one player to your roster before saving."); return; }
    try {
      setSaving(true);
      await api.put(`/leagues/${currentTeam.leagueId}/teams/${currentTeam._id}`, { players: roster });
      await fetchTeams();
      alert("Team saved successfully!");
    } catch (err) { console.error('Save error:', err); alert(err.response?.data?.message || "Failed to save team."); }
  ...
  const handleFormationChange = (formation) => {
    if (sport === 'soccer') {
      ...
      const newPlayersMap = {}
      newPositions.forEach(pos => { newPlayersMap[pos.id] = currentPlayersMap[pos.id] || null })
      setSoccerPlayers(newPlayersMap); setSoccerFormation(formation)
    } else {
      const newPositions = NBA_LINEUPS[formation]
      const currentPlayersMap = { ...nbaPlayers }
      const newPlayersMap = {}
      newPositions.forEach(pos => { newPlayersMap[pos.id] = currentPlayersMap[pos.id] || null })
      setNbaPlayers(newPlayersMap); setNbaLineup(formation)
    }
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 sm:mb-10 gap-4 sm:gap-6">
          <div>
            ...
            <p className="text-gray-500 font-medium italic text-sm sm:text-base">Configure your {sport} lineup for the next matchday.</p>
            <p className="text-xs text-gray-600 mt-2">💡 Tap a slot to assign · long-press or right-click to remove</p>
          </div>
          <div className="flex p-1.5 bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl shadow-inner self-start sm:self-auto">
            <button onClick={() => setSport('soccer')}
              className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-2 ${sport === 'soccer' ? 'bg-green-500 text-gray-950 shadow-lg shadow-green-500/20' : 'text-gray-400 hover:text-white'}`}>
              <FaFutbol /> Soccer
            </button>
            <button onClick={() => setSport('basketball')}
              className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-2 ${sport === 'basketball' ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'text-gray-400 hover:text-white'}`}>
        ...
        {loading ? (
          <div className="bg-gray-900 border border-gray-800 rounded-[2rem] p-12 sm:p-16 text-center shadow-2xl">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-[2rem] p-12 sm:p-16 text-center shadow-2xl">
            <div className="text-5xl sm:text-7xl mb-6 opacity-80 animate-bounce">{sport === 'soccer' ? <FaFutbol /> : <FaBasketballBall />}</div>
            <h2 className="text-xl sm:text-2xl font-bold mb-3">No {sport} team detected</h2>
            <p className="text-gray-400 max-w-sm mx-auto mb-8 text-sm sm:text-base">You haven't initialized a squad for this sport yet. Let's get your first team on the field!</p>
            <button onClick={handleCreateTeam} className="bg-white text-gray-950 font-black px-8 sm:px-10 py-3 sm:py-4 rounded-2xl hover:scale-105 active:scale-95 transition shadow-xl">Initialize Team</button>
          </div>
        ) : (
          <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {/* Control Bar */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
              {/* Formation tabs — horizontally scrollable on mobile */}
                ...
                <div className="flex gap-1.5 p-1 bg-gray-900/50 rounded-xl border border-gray-800 w-max">
                  {(sport === 'soccer' ? Object.keys(FORMATIONS) : Object.keys(NBA_LINEUPS)).map(f => (
                    <button key={f} onClick={() => handleFormationChange(f)}
                      className={`px-2.5 py-1.5 rounded-lg text-xs font-bold transition whitespace-nowrap ${ (sport === 'soccer' ? soccerFormation : nbaLineup) === f ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}>
                      {f}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex gap-2 sm:gap-3 shrink-0">
                ...
                <button onClick={() => navigate('/search')} className="bg-white hover:bg-gray-200 text-gray-950 font-black px-4 sm:px-5 py-2.5 rounded-xl text-xs transition shadow-lg">
                  + Transfer
                </button>
              </div>
            </div>
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 text-center">
              <p className="text-xs text-blue-400">
                ...
                {sport === 'soccer' && ` • ${positions.length} Players`}
            ...
            <div className="relative">
              {sport === 'soccer' ? (
                <SoccerPitch positions={positions} players={currentPlayers} onSlotClick={openPicker}
                  onDragStart={handleDragStart} onDragOver={() => {}} onDrop={handleDrop} onRemovePlayer={removePlayer} />
              ) : (
                <NBACourt positions={positions} players={currentPlayers} onSlotClick={openPicker}
                  onDragStart={handleDragStart} onDragOver={() => {}} onDrop={handleDrop} onRemovePlayer={removePlayer} />
              )}
              
              {/* Summary Card */}
              <div className="mt-6 sm:mt-8 bg-gray-900/40 border border-white/5 rounded-2xl p-4 flex items-center justify-between backdrop-blur-sm">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="h-9 w-9 sm:h-10 sm:w-10 bg-gray-800 rounded-full flex items-center justify-center font-bold text-base sm:text-lg text-green-500">
                    {playersCount}
                  </div>
                  <div>
        ...
        {/* Player Picker Modal */}
        {selectedSlot && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-end sm:items-center justify-center z-50 p-0 sm:p-6 animate-in fade-in duration-300">
            <div className="bg-gray-900 border border-white/10 rounded-t-[2.5rem] sm:rounded-[2.5rem] w-full sm:max-w-md overflow-hidden shadow-3xl">
              <div className="p-5 sm:p-6 border-b border-white/5 flex justify-between items-center bg-gray-900/50">
                <div>
                  <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1">Squad Selection</p>
                  <p className="text-lg sm:text-xl font-bold">Pick your {selectedSlot.label}</p>
                </div>
                  ...
                  <div className="space-y-2">
                    {availablePlayers.map(p => (
                      <div key={p._id} onClick={() => assignPlayer(selectedSlot, p)}
                        className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 hover:bg-white/5 rounded-2xl cursor-pointer transition group border border-transparent hover:border-white/5">
                        <div className="w-11 h-11 sm:w-12 sm:h-12 bg-gray-800 rounded-full overflow-hidden border border-white/10 group-hover:border-green-500 transition-colors shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-black text-white group-hover:text-green-400 transition-colors truncate">{p.name}</p>
                        ...
                        <div className="h-8 w-8 rounded-full border border-white/10 flex items-center justify-center text-green-500 opacity-0 group-hover:opacity-100 transition-all shrink-0">+</div>
                      </div>
                    ))}
                  </div>
                )}
