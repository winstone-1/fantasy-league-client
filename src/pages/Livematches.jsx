...
const getStatusPill = (status, minute, sport) => {
  ...
  if (s === 'ht') return { label: 'HT', bg: 'bg-purple-600', dot: false }
  if (s === 'ft' || s === 'final') return { label: 'Final', bg: 'bg-gray-600', dot: false }
  if (s === 'live' || s === 'in_play' || s === '1h' || s === '2h') {
    const label = sport === 'basketball' && minute ? `${minute}` : minute ? `${minute}'` : 'LIVE'
    return { label, bg: 'bg-green-500', dot: true }
  }
  if (s === 'scheduled' || s === 'ns' || s === 'tbd') return { label: 'Soon', bg: 'bg-gray-700', dot: false }
  return { label: status ?? '—', bg: 'bg-gray-700', dot: false }
}

const sportIcon = (sport) => sport === 'basketball' ? <FaBasketballBall /> : <FaFutbol />
const teamInitials = (name = '') => name.slice(0, 3).toUpperCase()
const getTeamName = (team) => typeof team === 'string' ? team : team.name

const teamColor = (name = '') => {
  const palette = ['bg-red-700','bg-blue-700','bg-emerald-700','bg-violet-700','bg-amber-700','bg-cyan-700','bg-rose-700','bg-indigo-700']
  let hash = 0
...
const TickerCard = ({ match, active, onClick }) => {
  ...
  return (
    <button onClick={onClick}
      className={`shrink-0 w-36 sm:w-44 rounded-2xl p-3 sm:p-3.5 text-left transition-all duration-200 border
        ${active ? 'bg-gray-800 border-green-500/60 shadow-lg shadow-green-500/10' : 'bg-gray-900 border-gray-800 hover:border-gray-600'}`}>
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <span className="text-base sm:text-lg">{sportIcon(match.sport)}</span>
...
const TeamAvatar = ({ name, size = 'lg' }) => {
  ...
  const sizeClass = size === 'lg' ? 'w-14 h-14 sm:w-20 sm:h-20 text-lg sm:text-xl' : 'w-10 h-10 text-sm'
  return (
...
const PlayerRow = ({ player }) => {
  ...
  return (
    <div className="flex items-center justify-between bg-green-500/10 border border-green-500/20 rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3">
      <div className="flex items-center gap-2 sm:gap-3">
        <div className={`w-9 h-9 sm:w-10 sm:h-10 ${color} rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0`}>
      ...
      <div className="text-right">
        <p className="text-green-400 font-bold text-lg sm:text-xl leading-none">{player.points ?? 0}</p>
    ...
    <div className="bg-gray-900/80 border border-gray-800 rounded-3xl overflow-hidden">
      <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b border-gray-800">
        <div className="flex items-center gap-2 sm:gap-2.5">
          <span className="text-lg sm:text-xl">{sportIcon(match.sport)}</span>
          <div>
            <p className="text-white font-semibold text-sm">{match.sport === 'basketball' ? 'NBA' : 'Premier League'}</p>
            <p className="text-gray-500 text-xs">{myPlayers.length} of your player{myPlayers.length !== 1 ? 's' : ''} active</p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between px-6 sm:px-10 py-6 sm:py-8">
        <div className="flex flex-col items-center gap-2 sm:gap-3">
          ...
          <p className="text-white font-semibold text-xs sm:text-sm tracking-wide">{teamInitials(homeTeamName)}</p>
          <p className="text-white font-black text-4xl sm:text-5xl leading-none">{match.homeScore ?? 0}</p>
        </div>
        <span className="text-gray-700 font-black text-xl sm:text-2xl tracking-widest">VS</span>
        <div className="flex flex-col items-center gap-2 sm:gap-3">
          <TeamAvatar name={awayTeamName} size="lg" />
          <p className="text-white font-semibold text-xs sm:text-sm tracking-wide">{teamInitials(awayTeamName)}</p>
          <p className="text-white font-black text-4xl sm:text-5xl leading-none">{match.awayScore ?? 0}</p>
        </div>
      </div>
        ...
        <div className="px-4 sm:px-5 pb-4 sm:pb-5">
          <div className="border-t border-gray-800 pt-4 mb-3">
            <div className="flex items-center gap-2 text-gray-400 text-sm font-medium">
              <span className="text-green-500">⚡</span> Your Players in This Game
            </div>
          </div>
          <div className="space-y-2">
            {myPlayers.map((p, i) => <PlayerRow key={i} player={p} />)}
          </div>
        </div>
      )}
    </div>
  ...
  <div className="bg-gray-900 border border-gray-800 rounded-3xl p-5 animate-pulse space-y-4">
    ...
    <div className="flex items-center justify-between px-6 sm:px-10">
      <div className="flex flex-col items-center gap-2">
        <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-full bg-gray-800" />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Live Scores</h1>
              {liveCount > 0 && (
          ...
          {lastPoll && (
            <p className="text-gray-600 text-xs mt-1 shrink-0">
        ...
        {!loading && matches.length > 0 && (
          <div className="bg-gray-900/60 border border-gray-800 rounded-3xl p-4 sm:p-5">
            ...
            <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-1 scrollbar-hide">
              {matches.map(match => (
                <TickerCard key={match._id} match={match} active={match._id === selected} onClick={() => setSelected(match._id)} />
              ))}
            </div>
          </div>
        )}
        ...
        {!loading && matches.length > 0 && (
          <div className="space-y-4">
            {[...(activeMatch ? [activeMatch] : []), ...matches.filter(m => m._id !== selected)].map(match => (
